...
2. **Fix the unambiguous** (index/log sync, obvious broken links, filename casing, missing `type:`
   ...
   structure — a missing or misordered heading).
...
Every concept is OKF-conformant and matches its type's template, each bundle's `index.md`/`log.md`
match its concepts, no dangling links remain unhandled, and open issues are surfaced.
